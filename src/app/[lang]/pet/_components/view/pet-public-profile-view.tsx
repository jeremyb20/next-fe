...
export default function PetPublickProfileView({ petProfile, canEdit }: Props) {
  ...
  // Estados para el consentimiento de ubicación
  ...
  useEffect(() => {
    ...
    // Verificar si ya se registró la ubicación en esta sesión
    ...
    if (hasRecordedLocation === 'true') {
      ...
      // Solo mostrar el overlay si showLocationConsent es true
      consentRequestedRef.current = true;
      // No hacer nada, el overlay se mostrará automáticamente
    } else if (!canShowLocationConsent) {
      // Si no requiere consentimiento, aceptar automáticamente
      setLocationConsent('accepted');
    }
  }, [petProfile?.memberPetId, canShowLocationConsent]);
  ...
  return (
    <>
      {/* Overlay de consentimiento - solo se muestra si showLocationConsent es true y el estado es pending */}
      {canShowLocationConsent && (
      ...
      {/* Contenido principal - solo visible si la ubicación fue aceptada o no requiere consentimiento */}
      {(locationConsent === 'accepted' || !canShowLocationConsent) && (
        <Container
